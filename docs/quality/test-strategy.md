...
- Golden linguistic fixtures pin the PHOIBLE snapshot and expected linguistic output. Run
  manifests record the observed eSpeak NG version. Production Python containers pin the exact
  eSpeak NG package revision from their digest-pinned Ubuntu base; host and scheduled jobs use
  the current package from the selected runner OS repository. A platform or package change that
  alters output requires scientific review rather than a blind snapshot update.
