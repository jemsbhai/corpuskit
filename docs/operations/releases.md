...
All Docker base-image references are pinned to OCI index digests in the Dockerfiles. The build
still consumes Debian package repositories while installing system packages, so a later rebuild
is not claimed to be bit-for-bit reproducible. The control is to build once, record provenance,
test the resulting digest, and promote that digest unchanged. Upstream Python and Node official
images currently have no CorpusKit-owned Sigstore identity policy in this repository; approving
new base digests remains an independently reviewed dependency change.
