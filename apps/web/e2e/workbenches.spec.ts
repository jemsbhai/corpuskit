...
async function mockApi(
  ...
  // Install the deterministic session response at the browser fetch boundary. Chromium
  // can report a successfully fulfilled Playwright route as net::ERR_ABORTED after the
  // response was consumed, which makes requestfailed unsuitable for synthetic fetches.
  // Real authentication remains covered in auth.spec.ts; all other traffic still uses
  // the browser network stack and remains subject to the strict failure listener below.
  await page.addInitScript((body) => {
    const nativeFetch = window.fetch.bind(window);
    let handledSessionRequests = 0;
    Object.defineProperty(window, "__corpuskitMockSessionRequests", {
      configurable: false,
      get: () => handledSessionRequests,
    });
  ...
  test(`${route} is accessible, keyboard reachable, and responsive at 200% text`, async ({
    ...
    page.on("request", (request) => {
      ...
      if (url.pathname === "/auth/session") {
        boundaryLeaks.push(
          `network session: ${request.method()} ${request.url()}`,
        );
      }
      if (
        route === "/jobs" &&
        ...
        boundaryLeaks.push(
          `project prefetch: ${request.method()} ${request.url()}`,
        );
      }
    });
    ...
    // Prove the application consumed the in-browser session boundary exactly once. A
    // network request here would bypass that deterministic boundary and is rejected below.
      ...
      .poll(() =>
        page.evaluate(
          () =>
            (
              window as Window & {
                readonly __corpuskitMockSessionRequests?: number;
              }
            ).__corpuskitMockSessionRequests ?? 0,
        ),
      )
      .toBe(1);
