...
class _UnusedSelectionResultArtifactStager:
    """Keep the EVALUATE-only memory fixture spawn-safe and fail closed on drift."""
    ...
    def stage_model_result(
        ...
        *,
        ...
        del kind, payload, content_sha256
        raise AssertionError("performance memory fixture unexpectedly staged a selection result")
...
async def _memory_probe(
    ...
    runner = ProcessExecutionRunner(
        build_core_handler_registry(settings, stager=_UnusedSelectionResultArtifactStager()),
