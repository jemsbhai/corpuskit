...
def test_checkpoint_rejects_pickle_weights_and_compatibility_mismatch(tmp_path: Path) -> None:
    location, pin = snapshot(tmp_path)
    bindings = FakeTrainingBindings()
...
def test_training_requires_resolver_supported_prompt_and_consistent_steps(tmp_path: Path) -> None:
    location, pin = snapshot(tmp_path)
...
def test_snapshot_invalid_json_and_empty_checkpoint_fail_closed(tmp_path: Path) -> None:
    location, _ = snapshot(tmp_path)
