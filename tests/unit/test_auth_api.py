...
from corpuskit.domain.phon_rl import PhonRlKlRequest, PhonRlMatrixResult
...
class FixedPhonRlLab:
    """Dependency-free lab seam for authentication and authorization tests."""

    def kl_penalty(self, request: PhonRlKlRequest) -> PhonRlMatrixResult:
        assert request.policy_log_probs.values == ((0.0,),)
        assert request.reference_log_probs.values == ((0.0,),)
        return PhonRlMatrixResult(values=((0.0,),))
...
def _client(role: AuthRole) -> httpx.AsyncClient:
    ...
    app = create_app(
        ...
        phon_rl_lab_service_factory=lambda _: FixedPhonRlLab(),  # type: ignore[arg-type]
    )
