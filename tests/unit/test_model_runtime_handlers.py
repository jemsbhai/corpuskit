...
import corpuskit.worker.model_registry as model_registry_module
...
from corpuskit.workflows.trusted_inputs import TrustedPeftAdapterInput, TrustedRunInputs
...
class RecordingEngine:
    ...
    def run_local_phon_rl(
        ...
        *,
        ...
        del request, policy, profile
        assert adapter_root.name == "adapter"
        assert compatibility.peft_adapter is True
        return local_result()
...
def test_local_adapter_executes_only_from_matching_parent_authorized_materialization(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    artifact_id = uuid4()
    ...
    request = local_request().model_copy(
        update={
    ...
    trusted = TrustedRunInputs(
        ...
        peft_adapter=TrustedPeftAdapterInput(
            ...
            artifact_sha256=artifact_sha256,
            checkpoint_sha256=checkpoint_sha256,
            materialized_manifest_sha256="e" * 64,
        ),
    )
    compatibility = PhonRlCheckpointCompatibility(
    ...
    adapter_root.mkdir()
    monkeypatch.setattr(model_registry_module, "claim_trusted_input", lambda *_, **__: tmp_path)
    monkeypatch.setattr(
        model_registry_module,
        "read_materialized_peft_manifest",
        lambda *_: (adapter_root, SimpleNamespace(compatibility=compatibility)),
    )
    runtime = ModelRuntimeCoordinator(
        ModelRuntimePolicy(
            local_models=(local_policy().model_copy(update={"allow_phon_rl_adapters": True}),),
            worker_profile=WorkerModelProfile.LOCAL_CPU,
        ),
        RecordingEngine(),
    )
    handler = LocalGenerationDurableHandler(runtime, MemoryArtifactStager(), tmp_path)

    with pytest.raises(EngineContractError) as missing_materialization:
        handler.execute(spec)
    assert (
        missing_materialization.value.operation
        == "model_runtime.local.phon_rl_materialization_required"
    )
    with pytest.raises(EngineContractError) as malformed_claim:
        handler.execute_with_trusted_inputs(spec, {}, None)
    assert malformed_claim.value.operation == "model_runtime.local.phon_rl_claim"
    with pytest.raises(EngineContractError) as wrong_spec:
        handler.execute_with_trusted_inputs(
            spec,
            trusted.model_copy(update={"spec_sha256": "f" * 64}).model_dump(mode="json"),
            None,
        )
    assert wrong_spec.value.operation == "model_runtime.local.phon_rl_claim"
    assert trusted.peft_adapter is not None
    mismatched_adapter = trusted.peft_adapter.model_copy(update={"artifact_sha256": "f" * 64})
    with pytest.raises(EngineContractError) as wrong_adapter:
        handler.execute_with_trusted_inputs(
            spec,
            trusted.model_copy(update={"peft_adapter": mismatched_adapter}).model_dump(mode="json"),
            None,
        )
    assert wrong_adapter.value.operation == "model_runtime.local.phon_rl_claim"

    summary = handler.execute_with_trusted_inputs(
        spec,
        trusted.model_dump(mode="json"),
        None,
    )

    assert summary["schema_id"] == "corpuskit.local-generation-result.v1"
...
def test_durable_artifact_contract_rejects_non_content_addressed_or_oversized_results() -> None:
    ...
    class FailingStager:
        def stage_model_result(self, **_: object) -> str:
            raise OSError("private staging failure")

    with pytest.raises(EngineUnavailableError) as staging_failure:
        _stage(
            FailingStager(),
            RunKind.GENERATE_LOCAL,
            b"bounded",
    ...
    assert staging_failure.value.operation == "model_runtime.artifact.staging"

    class MalformedReferenceStager:
        def stage_model_result(self, **_: object) -> str:
            return "not-a-staged-artifact-reference"

    with pytest.raises(EngineContractError) as malformed_reference:
        _stage(
            MalformedReferenceStager(),
            RunKind.GENERATE_LOCAL,
            b"bounded",
            schema_id="corpuskit.local-generation-result.v1",
        )
    assert malformed_reference.value.operation == "model_runtime.artifact.staging_reference"
