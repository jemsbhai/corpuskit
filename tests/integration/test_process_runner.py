...
class ProgressThenResultHandler:
    ...
    def execute_with_progress(
        ...
        if self.release_marker is None:
            ...
            while not self.release_marker.exists():
                time.sleep(0.01)
...
class RecordingChildConnection:
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_progress_crosses_ipc_before_child_completion(tmp_path: Path) -> None:
    seen: list[DurableRunProgress] = []
    progress_visible = asyncio.Event()
    release_marker = tmp_path / "allow-child-completion"
    runner = ProcessExecutionRunner(
        HandlerRegistry((ProgressThenResultHandler(release_marker=release_marker),)),
        hard_timeout_seconds=5,
    )
    ...
    progress_wait = asyncio.create_task(progress_visible.wait())
    try:
        completed, _ = await asyncio.wait(
            {execution, progress_wait},
            timeout=15,
            return_when=asyncio.FIRST_COMPLETED,
        )
        if not completed:
            pytest.fail("child did not publish progress within the bounded IPC deadline")
        if execution in completed and progress_wait not in completed:
            await execution
            pytest.fail("child completed without publishing progress")
        assert progress_wait in completed
    ...
    assert execution.result() == {"artifact_count": 1}
...
@pytest.mark.parametrize(
    ...
    [
        b"[]",
        b'{"kind":"export","spec":[]}',
        b'{"kind":"export","spec":{},"trusted_inputs":[]}',
    ],
)
def test_child_entrypoint_sanitizes_malformed_envelopes_and_pipe_failures(
    request_bytes: bytes,
) -> None:
    ...
    assert connection.messages == [b'{"code":"internal_error","retryable":true,"status":"error"}']

    failed_connection = RecordingChildConnection(fail_send=True)
    _child_execute(
        HandlerRegistry((ResultHandler({"artifact_count": 1}),)),
        request_bytes,
...
def test_oversized_progress_envelope_is_rejected_before_payload_parsing() -> None:
    with pytest.raises(RunExecutionError) as nonterminal:
        _decode_response(b'{"status":"progress","progress":{"sequence":0,"phase":"generating"}}')
    assert nonterminal.value.code == "worker_process_contract"
