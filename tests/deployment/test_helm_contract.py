...
def test_deployment_workflow_installs_espeak_before_running_tests() -> None:
    workflow = DEPLOYMENT_WORKFLOW.read_text(encoding="utf-8")
    install_marker = "      - name: Install eSpeak NG\n"
    pytest_marker = "      - name: Validate chart and negative policy tests\n"

    assert workflow.index(install_marker) < workflow.index(pytest_marker)
    install = workflow.split(install_marker, maxsplit=1)[1].split("\n      - name:", maxsplit=1)[0]
    assert "sudo apt-get update" in install
    assert "sudo apt-get install --yes --no-install-recommends espeak-ng" in install
